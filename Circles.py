...
class CirclesMod(loader.Module):
	...
	async def roundcmd(self, message):
		"""(Реплай на изображение/стикер или видео/gif)"""
		reply = None
		if message.is_reply:
			reply = await message.get_reply_message()
			data = await check_media(reply)
			if isinstance(data, bool):
				await utils.answer(message, "<b>Реплай на изображение/стикер или видео/gif!</b>")
				return
		else:
			await utils.answer(message, 
"<b>Реплай на изображение/стикер или видео/gif!</b>")
			return
		data, type = data
		if type == "img":
			await message.edit("<b>Обработка изображения</b>📷")
			img = io.BytesIO()
			...
			await message.edit("<b>Сохранение видео</b>📼")
